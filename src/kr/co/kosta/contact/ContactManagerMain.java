...
public class ContactManagerMain {
	...
	public static void main(String[] args) throws IOException {
		 ...
		 * >> 선택
		 * 
		 * EX > 1. 선택 -> 사용자이름 이메일 나이 주소를 각각 입력해주세요. 각각 입력해 주세요. 구분자는 스페이스입니다.
		 * 
		 */
		String choice = "";
		do {
			...
			choice = br.readLine();
			ContactService_DB ss = new MySQLContactService();
	...
	/*
	 * public static void createUser() throws IOException{ BufferedReader
	 * reader2 = new BufferedReader(new InputStreamReader(System.in)); try{
	 * BufferedWriter bW; FileWriter fw; fw = new FileWriter("test.txt", false);
	 * bW = new BufferedWriter(fw);
	 * 
	 * System.out.println("사용자이름 이메일 나이 주소를 각각 입력해주세요. 구분자는 스페이스입니다."); String
	 * user= reader2.readLine(); String[] result = user.split(" ",4);
	 * 
	 * bW.write("사용자이름 : "+result[0]+" 이메일 :"+result[1]+" 나이 :"+result[2]
	 * +" 주소 :"+result[3]); bW.newLine(); //줄바꿈 bW.close(); }catch(IOException
	 * io){ System.out.println("error"); v } } public static void readUser()
	 * throws IOException{ try{ BufferedReader reader = new BufferedReader(new
	 * FileReader("test.txt"));
	 * 
	 * String txt = "";
	 * 
	 * while((txt = reader.readLine()) != null) { System.out.println(txt); }
	 * 
	 * reader.close(); }catch(IOException io){ System.out.println("error"); } }
	 */
}